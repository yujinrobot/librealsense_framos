    ...
    for (auto&& dev : devices_list)
    {
        ...
        if (dev.supports(RS2_CAMERA_INFO_PRODUCT_LINE) &&
            ...
            ((std::string(dev.get_info(RS2_CAMERA_INFO_PRODUCT_LINE)) == "D400") ||
             (std::string(dev.get_info(RS2_CAMERA_INFO_PRODUCT_LINE)) == "D400e")))
#elif
            std::string(dev.get_info(RS2_CAMERA_INFO_PRODUCT_LINE)) == "D400")
#endif
        {
...
#ifdef FRAMOS
    // enable metadata on FRAMOS D400e cameras
    if (depth_sensor.supports(RS2_OPTION_METADATA_TOGGLE)) {
        depth_sensor.set_option(RS2_OPTION_METADATA_TOGGLE, 1);
    }
#endif
