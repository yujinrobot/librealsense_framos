...
_type
{
    RS2_D400E_PORT_TYPE_ALL,
    RS2_D400E_PORT_TYPE_CONTROL,
    RS2_D400E_PORT_TYPE_STREAM,
    RS2_D400E_PORT_TYPE_MESSAGE,
    RS2_D400E_PORT_TYPE_COUNT
} rs2_d400e_port_type;
const char* rs2_d400e_port_type_to_string(rs2_d400e_port_type type);
#endif
...
#ifdef FRAMOS
typedef struct rs2_cli_arg
{
    int argc;
    const char** argv;
} rs2_cli_arg;

typedef struct rs2_d400e_filter_serial_range
{
    unsigned long long begin;
    unsigned long long end;
} rs2_d400e_filter_serial_range;

typedef struct rs2_d400e_filter_ip_range
{
    unsigned int begin;
    unsigned int end;
} rs2_d400e_filter_ip_range;

typedef struct rs2_d400e_filter
{
    // Specify either serial_range or ip_range 
    // (serial_range has higher priority, therefore serial_range will be used if both were specified)
    rs2_d400e_filter_serial_range serial_range;
    rs2_d400e_filter_ip_range ip_range;
} rs2_d400e_filter;
#endif
