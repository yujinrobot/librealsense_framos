...
namespace librealsense
{
    ...
    void cs_advanced_mode_base::get_depth_inter_packet_delay(depth_inter_packet_delay_control* ptr) const
    {
        if (supports_option(*_depth_sensor, RS2_OPTION_INTER_PACKET_DELAY))
        {
            ptr->depth_inter_packet_delay = _depth_sensor->get_option(RS2_OPTION_INTER_PACKET_DELAY).query();
            ptr->was_set = true;
        }
    }

    void cs_advanced_mode_base::get_color_inter_packet_delay(color_inter_packet_delay_control* ptr) const
    {
        if (*_color_sensor && supports_option(**_color_sensor, RS2_OPTION_INTER_PACKET_DELAY))
        {
            ptr->color_inter_packet_delay = (*_color_sensor)->get_option(RS2_OPTION_INTER_PACKET_DELAY).query();
            ptr->was_set = true;
        }
    }

    void cs_advanced_mode_base::get_depth_packet_size(depth_packet_size_control* ptr) const
    {
        if (supports_option(*_depth_sensor, RS2_OPTION_INTER_PACKET_DELAY))
        {
            ptr->depth_packet_size = _depth_sensor->get_option(RS2_OPTION_PACKET_SIZE).query();
            ptr->was_set = true;
        }
    }

    void cs_advanced_mode_base::get_color_packet_size(color_packet_size_control* ptr) const
    {
        if (*_color_sensor && supports_option(**_color_sensor, RS2_OPTION_PACKET_SIZE))
        {
            ptr->color_packet_size = (*_color_sensor)->get_option(RS2_OPTION_PACKET_SIZE).query();
            ptr->was_set = true;
        }
    }
    ...
    void cs_advanced_mode_base::set_depth_inter_packet_delay(const depth_inter_packet_delay_control& val)
    {
        if (val.was_set)
            _depth_sensor->get_option(RS2_OPTION_INTER_PACKET_DELAY).set(val.depth_inter_packet_delay);
    }

    void cs_advanced_mode_base::set_color_inter_packet_delay(const color_inter_packet_delay_control& val)
    {
        if (val.was_set && !*_color_sensor)
            throw invalid_value_exception("Can't set color_inter_packet_delay value! Color sensor not found.");
        if (val.was_set)
            (*_color_sensor)->get_option(RS2_OPTION_INTER_PACKET_DELAY).set(val.color_inter_packet_delay);
    }

    void cs_advanced_mode_base::set_depth_packet_size(const depth_packet_size_control& val)
    {
        if (val.was_set && !_depth_sensor->is_streaming())
            _depth_sensor->get_option(RS2_OPTION_PACKET_SIZE).set(val.depth_packet_size);
    }

    void cs_advanced_mode_base::set_color_packet_size(const color_packet_size_control& val)
    {
        if (val.was_set && !*_color_sensor)
            throw invalid_value_exception("Can't set color_packet_size value! Color sensor not found.");
        if (val.was_set && !(*_color_sensor)->is_streaming())
            (*_color_sensor)->get_option(RS2_OPTION_PACKET_SIZE).set(val.color_packet_size);
    }
    ...
    preset cs_advanced_mode_base::get_all() const
    {
        ...
        get_color_auto_white_balance(&p.color_auto_white_balance);
        get_color_power_line_frequency(&p.color_power_line_frequency);
        get_depth_inter_packet_delay(&p.depth_inter_packet_delay);
        get_color_inter_packet_delay(&p.color_inter_packet_delay);
        get_depth_packet_size(&p.depth_packet_size);
        get_color_packet_size(&p.color_packet_size);
        return p;
    }
    ...
    void cs_advanced_mode_base::set_all(const preset& p)
    {
        ...
        set_depth_inter_packet_delay(p.depth_inter_packet_delay);
        set_color_inter_packet_delay(p.color_inter_packet_delay);
        set_depth_packet_size(p.depth_packet_size);
        set_color_packet_size(p.color_packet_size);
