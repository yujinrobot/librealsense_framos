...
#ifdef FRAMOS
#define SAMPLES_BASE_UPDATE_PERIOD_MS (60000) //1 minute in ms
...
namespace librealsense
{
    ...
    bool CLinearCoefficients::update_samples_base(double x)
    {
        ...
        if ((_last_values.front()._x - x) > max_device_time / 2)
            ...
            base_x = -max_device_time;
#ifdef FRAMOS
        else if ((x - _base_sample._x) > SAMPLES_BASE_UPDATE_PERIOD_MS)
        {
            update_samples_base_periodically();
            return true;
        }
#endif
        else
...
#ifdef FRAMOS
    bool CLinearCoefficients::update_samples_base_periodically()
    {
        if (_last_values.empty())
            return false;
...
#ifdef FRAMOS
    bool time_diff_keeper::is_active_object_active() {
        return _active_object.is_active();
    }

    void time_diff_keeper::active_object_stop() {
        _active_object.stop();
    }
#endif
    ...
    {
        bool stop = true;
        d400e_camera* d400e = dynamic_cast<d400e_camera*>(_device);
        if (d400e != nullptr) {
            stop = false;
        }
