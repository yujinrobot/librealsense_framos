...
/** \brief Metadata fields that are utilized internally by librealsense
    ...
    */
    ...
    /**\brief A CS UVC-Header parser class*/
    template<class St, class Attribute>
    class md_cs_uvc_header_parser : public md_attribute_parser_base
    {
    public:
        md_cs_uvc_header_parser(Attribute St::* attribute_name, attrib_modifyer mod) :
            _md_attribute(attribute_name), _modifyer(mod) {};
        ...
        bool supports(const librealsense::frame& frm) const override
        {
            return (frm.additional_data.metadata_size >= framos_uvc_header_size);
        }

    private:
        md_cs_uvc_header_parser() = delete;
        md_cs_uvc_header_parser(const md_cs_uvc_header_parser&) = delete;

        Attribute St::* _md_attribute;  // Pointer to the attribute within uvc header that provides the relevant data
        attrib_modifyer     _modifyer;      // Post-processing on received attribute
    };

    /**\brief A utility function to create CS UVC metadata header parser*/
    template<class St, class Attribute>
    std::shared_ptr<md_attribute_parser_base> make_cs_uvc_header_parser(Attribute St::* attribute, attrib_modifyer mod = nullptr)
    {
        std::shared_ptr<md_cs_uvc_header_parser<St, Attribute>> parser(new md_cs_uvc_header_parser<St, Attribute>(attribute, mod));
        return parser;
    }
    ...
    class cs_md_attribute_actual_fps : public md_attribute_parser_base
    {
    public:
        cs_md_attribute_actual_fps(bool discrete = true, attrib_modifyer  exposure_mod = [](const rs2_metadata_type& param) {return param; })
            : _fps_values{ 6, 15, 30, 60, 90 }, _exposure_modifyer(exposure_mod), _discrete(discrete)
        {}
