...
#pragma pack(push, 1)
    ...
    // NH - From D4 v5.13.00.50 - MIPI out axis - 59 tvalids, each 4 bytes, total = 236 bytes
    const uint8_t FRAMOS_METADATA_SIZE = 236;
    ...
    typedef platform::uvc_header framos_uvc_header;

    constexpr uint8_t framos_uvc_header_size = sizeof(framos_uvc_header);

    struct metadata_framos_basic
    {
        //platform::cs_header     header;
        framos_uvc_header       header;
        md_capture_timing       capture_timing;
        md_capture_stats        capture_stats;
        md_depth_control        depth_control;
