    ...
    namespace pipeline
    {
        ...
        void pipeline::unsafe_stop()
        {
            if (_active_profile)
            {
                ...
                // For FRAMOS camera, dispatcher was reallocated from try/catch block, due to the possible
                // exceptions on the stream stop/close. In that case dispatcher will remain active. Moving the 
                // dispatcher.stop() ensures the clean stop.
                try
                {
                    _dispatcher.stop();
                }
        ...
        std::vector<int> pipeline::on_start(std::shared_ptr<profile> profile)
        {
            ...
            {
                _syncer = std::unique_ptr<syncer_process_unit>(new syncer_process_unit({}, true,_syncer_mode, matchers));
