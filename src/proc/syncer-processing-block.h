...
namespace librealsense
{
    ...
    public:
#ifdef FRAMOS
        syncer_process_unit(std::initializer_list< bool_option::ptr > enable_opts, bool log = true, rs2_syncer_mode syncer_mode = RS2_SYNCER_MODE_DEFAULT, std::vector<std::shared_ptr<matcher>> matchers = {} );

        syncer_process_unit( bool_option::ptr is_enabled_opt = nullptr, bool log = true, rs2_syncer_mode syncer_mode = RS2_SYNCER_MODE_DEFAULT, std::vector<std::shared_ptr<matcher>> matchers = {})
            : syncer_process_unit( { is_enabled_opt }, log, syncer_mode, matchers) {}
#endif
#ifndef FRAMOS
