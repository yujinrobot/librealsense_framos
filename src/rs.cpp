...
#ifdef FRAMOS
struct rs2_d400e_port_list
{
    std::vector<uint16_t> ports;
};

struct rs2_d400e_filter_list
{
...
void rs2_d400e_get_port_range(unsigned short* min, unsigned short* max, rs2_error** error) BEGIN_API_CALL
{
    librealsense::d400e::get_port_range(min, max);
}
HANDLE_EXCEPTIONS_AND_RETURN(, min, max)

void rs2_d400e_set_port_range(unsigned short min, unsigned short max, rs2_error** error) BEGIN_API_CALL
{
    librealsense::d400e::set_port_range(min, max);
}
HANDLE_EXCEPTIONS_AND_RETURN(, min, max)

rs2_d400e_port_list* rs2_d400e_query_ports(rs2_error** error) BEGIN_API_CALL
{
    return new rs2_d400e_port_list{ librealsense::d400e::query_ports(RS2_D400E_PORT_TYPE_ALL) };
}
NOARGS_HANDLE_EXCEPTIONS_AND_RETURN(nullptr)

rs2_d400e_port_list* rs2_d400e_query_ports_by_type(rs2_d400e_port_type type, rs2_error** error) BEGIN_API_CALL
{
    return new rs2_d400e_port_list{ librealsense::d400e::query_ports(type) };
}
HANDLE_EXCEPTIONS_AND_RETURN(nullptr, type)

unsigned int rs2_d400e_get_port_count(const rs2_d400e_port_list* ports, rs2_error** error) BEGIN_API_CALL
{
    VALIDATE_NOT_NULL(ports);
    return ports->ports.size();
}
HANDLE_EXCEPTIONS_AND_RETURN(0, ports)

unsigned short rs2_d400e_get_port(const rs2_d400e_port_list* ports, unsigned int index, rs2_error** error) BEGIN_API_CALL
{
    VALIDATE_NOT_NULL(ports);
    VALIDATE_RANGE(index, 0, ports->ports.size() - 1);

    return ports->ports[index];
}
HANDLE_EXCEPTIONS_AND_RETURN(0, ports, index)

void rs2_d400e_delete_port_list(rs2_d400e_port_list* ports) BEGIN_API_CALL
{
    VALIDATE_NOT_NULL(ports);
    delete ports;
}
NOEXCEPT_RETURN(, ports)

void rs2_d400e_set_cli_args(const rs2_cli_arg* cli_arg, rs2_error** error) BEGIN_API_CALL
{
    VALIDATE_NOT_NULL(cli_arg);

    librealsense::d400e::cli_args::get_instance().set(cli_arg);
}
HANDLE_EXCEPTIONS_AND_RETURN(, cli_arg)

rs2_d400e_filter_list* rs2_d400e_new_filter_list(rs2_error** error) BEGIN_API_CALL
{
    return new rs2_d400e_filter_list{};
}
NOARGS_HANDLE_EXCEPTIONS_AND_RETURN(nullptr)

void rs2_d400e_delete_filter_list(rs2_d400e_filter_list* filter_list) BEGIN_API_CALL
{
    VALIDATE_NOT_NULL(filter_list);
    delete filter_list;
}
NOEXCEPT_RETURN(, filter_list)

void rs2_d400e_filter_list_insert(const rs2_d400e_filter* filter, rs2_d400e_filter_list* filter_list, rs2_error** error) BEGIN_API_CALL
{
    VALIDATE_NOT_NULL(filter);
    VALIDATE_NOT_NULL(filter_list);

    rs2_d400e_filter* filter_tmp = const_cast<rs2_d400e_filter*>(filter);
    filter_list->filter_list.push_back(*filter_tmp);
}
HANDLE_EXCEPTIONS_AND_RETURN(, filter, filter_list)

void rs2_d400e_set_device_filter_list(const rs2_d400e_filter_list* list, rs2_error** error) BEGIN_API_CALL
{
    VALIDATE_NOT_NULL(list);

    librealsense::d400e::set_device_filter_list(list->filter_list);
}
HANDLE_EXCEPTIONS_AND_RETURN(, list)
#endif
...
rs2_processing_block* rs2_create_sync_processing_block(rs2_syncer_mode syncer_mode, rs2_error** error) BEGIN_API_CALL
{
    ...
    auto block = std::make_shared<librealsense::syncer_process_unit>(nullptr, true, syncer_mode);
