...
#define LOG_IF_ENABLE( OSTREAM, ENV ) \
...
#ifdef FRAMOS
    composite_matcher::composite_matcher(
        std::vector< std::shared_ptr< matcher > > const & matchers, std::string const & name, rs2_syncer_mode syncer_mode ) : _syncer_mode(syncer_mode)
#endif
    {
        for (auto&& matcher : matchers)
        {
        ...
        if (_syncer_mode == RS2_SYNCER_MODE_WAIT_FRAMESET)
        {
            for (auto s = _frames_queue.begin(); s != _frames_queue.end(); s++) {
                    ...
                    fold = & fh;}))
                {
                    ...
                    {
                        if (f.frame->get_frame_timestamp() > fold->frame->get_frame_timestamp()) {
...
#ifdef FRAMOS
    frame_number_composite_matcher::frame_number_composite_matcher(
        std::vector< std::shared_ptr< matcher > > const & matchers, rs2_syncer_mode syncer_mode)
    ...
    bool timestamp_composite_matcher::are_equivalent(frame_holder& a, frame_holder& b)
    {
        ...
        if (_syncer_mode == RS2_SYNCER_MODE_WAIT_FRAMESET)
        {
            double gap = 1000 / min_fps;
            if (a.frame->get_sensor()->supports_option(RS2_OPTION_AUTO_EXPOSURE_PRIORITY)) {
                if (!a.frame->get_sensor()->get_option(RS2_OPTION_AUTO_EXPOSURE_PRIORITY).query() && a_fps == b_fps) {
                    gap /= 2;
                }
            }
            else if (b.frame->get_sensor()->supports_option(RS2_OPTION_AUTO_EXPOSURE_PRIORITY)) {
                if (!b.frame->get_sensor()->get_option(RS2_OPTION_AUTO_EXPOSURE_PRIORITY).query() && a_fps == b_fps) {
                    gap /= 2;
                }
            }
...
#ifdef FRAMOS
    void timestamp_composite_matcher::update_next_expected( std::shared_ptr< matcher > const & matcher,
                                                           ...
                                                           const syncronization_environment & env )
    {
        ...
        // bugfix: matchers not deactivated if one stream has enabled and the other disabled global time enabled option and one stream is then disabled
        ...
        if( now > next_expected )
        {
            ...
            if (_syncer_mode == RS2_SYNCER_MODE_WAIT_FRAMESET)
            {
                ...
                //if (next_expected < (last_arrived.timestamp - gap)) //this is external event fix
                if (next_expected < (waiting_to_be_released->get_frame_timestamp() - gap)) //this is external event fix
                    return false;
            }
    ...
    bool timestamp_composite_matcher::are_equivalent( double a, double b, unsigned int fps )
    {
        ...
        if (_syncer_mode == RS2_SYNCER_MODE_WAIT_FRAMESET)
        {
            float gap_ee = 1000.f / fps;
            bool are_eq_ee = abs(a - b) < (gap);
            return are_eq_ee;
        }
    ...
    composite_identity_matcher::composite_identity_matcher(
        std::vector< std::shared_ptr< matcher > > const & matchers, rs2_syncer_mode syncer_mode)
        : composite_matcher(matchers, "CI: ", syncer_mode)
    {
    }
#endif
#ifndef FRAMOS
