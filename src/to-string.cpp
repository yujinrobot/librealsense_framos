...
#define CASE( X ) STRCASE( OPTION, X )
    ...
    CASE(EXTERNAL_EVENT_BURST_COUNT)
    CASE(RGB_LED_TOGGLE)
...
#ifdef FRAMOS
const char* get_string(rs2_d400e_port_type type)
{
#define CASE(X) STRCASE(D400E_PORT_TYPE, X)
    switch (type)
    {
        CASE(ALL)
            CASE(CONTROL)
...
const char* rs2_d400e_port_type_to_string(rs2_d400e_port_type type) { return librealsense::get_string(type); }
#endif
