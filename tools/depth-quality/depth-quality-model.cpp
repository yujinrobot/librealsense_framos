...
#ifdef FRAMOS
            std::regex FRAMOS_regex("FRAMOS ");
            std::string name = devices[0].get_info(RS2_CAMERA_INFO_NAME);
            if (std::regex_search(name, FRAMOS_regex))
            {
                {
                    rs2::config cfg_default;
                    // Preferred configuration Depth + Synthetic Color
                    cfg_default.enable_stream(RS2_STREAM_DEPTH, -1, 1280, 720, RS2_FORMAT_Z16, requested_fps);
                    cfg_default.enable_stream(RS2_STREAM_INFRARED, -1, 1280, 720, RS2_FORMAT_RGB8, requested_fps);
                    cfgs.emplace_back(cfg_default);
                }
                // Use Infrared luminocity as a secondary video in case synthetic chroma is not supported
                {
                    rs2::config cfg_alt;
                    cfg_alt.enable_stream(RS2_STREAM_DEPTH, -1, 1280, 720, RS2_FORMAT_Z16, requested_fps);
                    cfg_alt.enable_stream(RS2_STREAM_INFRARED, -1, 1280, 720, RS2_FORMAT_Y8, requested_fps);
...
#ifdef FRAMOS
            /*Set initial value of the selected resoulution*/
            auto depth_profile = _pipe.get_active_profile().get_stream(RS2_STREAM_DEPTH).as<video_stream_profile>();
            
            auto it = std::find(_depth_sensor_model->resolutions.begin(), _depth_sensor_model->resolutions.end(),
                      std::to_string(depth_profile.width()) + " x " + std::to_string(depth_profile.height()));
            if (it != _depth_sensor_model->resolutions.end())
            {
                _depth_sensor_model->ui.selected_res_id = (int)(it - _depth_sensor_model->resolutions.begin());
                _depth_sensor_model->last_valid_ui.selected_res_id = (int)(it - _depth_sensor_model->resolutions.begin());
            }
#endif    
