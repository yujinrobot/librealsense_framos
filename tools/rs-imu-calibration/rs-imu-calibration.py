...
def get_eeprom(calibration_table):
    ...
    return buffer
#FRAMOS
def write_eeprom_to_camera(active_device, eeprom, serial_no=''):
#END FRAMOS
    # DC_MM_EEPROM_SIZE = 520
...
#FRAMOS
    if active_device:
        debug = get_debug_device_helper(active_device)
    else:
...
#FRAMOS
def get_debug_device_helper(dev):
    # set to advance mode:
...
#FRAMOS
                ...
                if "FRAMOS" in dev.get_info(rs.camera_info.name):
                    product_line = 'D400'
                else:
...
#FRAMOS
        buckets_labels = [
        "Mounting screw (FRAMOS - SerialNumber label) pointing down, device facing out", 
        "Mounting screw (FRAMOS - SerialNumber label) pointing left, device facing out", 
        "Mounting screw (FRAMOS - SerialNumber label) pointing up, device facing out", 
        "Mounting screw (FRAMOS - SerialNumber label) pointing right, device facing out", 
...
#FRAMOS
            imu_wrap = imu_wrapper()
            if not imu_wrap.enable_imu_device(serial_no):
#END FRAMOS
                print('Failed to enable device.')
                return -1
#FRAMOS
            active_device = imu_wrap.get_active_device()
            measurements, gyro = imu_wrap.get_measurements(buckets, buckets_labels)
#END FRAMOS
...
#FRAMOS
                write_eeprom_to_camera(active_device, eeprom, serial_no)
#END FRAMOS
